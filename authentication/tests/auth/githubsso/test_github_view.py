...
class TestGithubSSOLoginView:
    ...
    @mock.patch('requests.get')
    @mock.patch('requests.post')
    def test_email_from_user_emails(self, mock_post, mock_get, mock_serializer, mock_refresh):
    ...
    @mock.patch('requests.get')
    @mock.patch('requests.post')
    def test_existing_inactive_user_reactivated(self, mock_post, mock_get, mock_serializer, mock_refresh, django_user_model):
        """User déjà existant et inactif : il doit être réactivé"""
        # Crée un user inactif
        user = django_user_model.objects.create(email='test@inactive.com', is_active=False)
        mock_post.return_value.json.return_value = {'access_token': 'token123'}
        mock_get.return_value.json.return_value = {'email': 'test@inactive.com', 'name': 'Inactive Guy'}
